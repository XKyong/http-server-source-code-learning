...
module.exports = function createMiddleware(_dir, _options) {
  ...
  if (opts.mimeTypes) {
    ...
    if (typeof opts.mimeTypes === 'string') {
      // mime 库最新版本跟当前这个项目所依赖的版本提供的 api 有出入，因此这里以 package-lock 中锁定的版本为主！
      // 地址：https://github.com/broofa/mime/blob/v1.6.0/README.md
  ...
  function shouldReturn304(req, serverLastModified, serverEtag) {
    ...
    if (clientModifiedSince) {
      ...
      try {
        clientModifiedDate = new Date(Date.parse(clientModifsortByIsDirectoryiedSince));
      } catch (err) {
        return false;
      }
  ...
  return function middleware(req, res, next) {
    ...
    // 对传入的文件 stat 信息进行处理，最终创建一个 读取流（ReadStream）将文件字节“流向”客户端！
    function serve(stat) {
      // Do a MIME lookup, fall back to octet-stream and handle gzip
      // and brotli special case.
      const defaultType = opts.contentType || 'application/octet-stream';
      // Get the mime type associated with a file, if no mime type is found, `defaultType` is returned
      ...
      if (contentType && isTextFile(contentType)) {
        // Buffer 类是全局可用的，不用单独引入也可以使用！
        // 但这里内置的 buffer 需要手动引入才能使用！
        // buffer.constants.MAX_LENGTH: 代表字符串能够拥有的最大长度，以 UTF-16 编码单位计算
        if (stat.size < buffer.constants.MAX_LENGTH) {
          const bytes = fs.readFileSync(file);
          // html-encoding-sniffer 库能够嗅探出 html 字节中的编码格式，比如这里的 sniffedEncoding 为 'UTF-8'
          // 地址：https://www.npmjs.com/package/html-encoding-sniffer
    ...
    // 获取访问 file 的 stat 信息，如果是文件夹，则递归去调用 middleware 函数，最终通过 showDir 方法处理文件夹信息；
    // 如果是 文件，调用 serve 方法将文件内容处理后返回给客户端。
    function statFile() {
      try {
        fs.stat(file, (err, stat) => {
          if (err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) {
            ...
            if (autoIndex) {
              middleware({
                // url-join 库能够将所有参数连接在一起，并对生成的 URL 进行规范化处理
                // 地址：https://www.npmjs.com/package/url-join
                url: urlJoin(
                ...
                // 如果传入的是 path 是 ./，且 index.html 和 404.html 文件均不存在，则该回调会作为 next 函数被执行！
                // 进而后续通过 showDir 方法将 ./ 作为文件夹处理！
                if (autoIndexError) {
